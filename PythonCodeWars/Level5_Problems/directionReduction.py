...
# "NORTH" and "SOUTH" are not directly opposite but they become directly opposite after the reduction 
...
def cantReduceNoMore(arr):
    for i in range(len(arr) - 1):
        if arr[i] == "NORTH" and arr[i+1] == "SOUTH" or arr[i] == "SOUTH" and arr[i+1] == "NORTH" :
            return False
        elif arr[i] == "WEST" and arr[i+1] == "EAST" or arr[i] == "EAST" and arr[i+1] == "WEST" :
            return False
    return True
...
def dirReduc(arr):
    i = 0
    while cantReduceNoMore(arr) == False:
        if i >= len(arr):
            i = 0
            continue
        elif i < len(arr) - 1 and (arr[i] == "NORTH" and arr[i+1] == "SOUTH" or arr[i] == "SOUTH" and arr[i+1] == "NORTH") :
            del arr[i]
            del arr[i]    
            continue
        elif i < len(arr) - 1 and (arr[i] == "WEST" and arr[i+1] == "EAST" or arr[i] == "EAST" and arr[i+1] == "WEST") :
            del arr[i]
            del arr[i]   
            continue
        else:
