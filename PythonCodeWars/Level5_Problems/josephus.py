...
def josephus(items,k):
    dead = []
    #start at the beinning of this pattern
    i = k - 1
    #continue looping until there's no one left
    while len(items) > 0:
        #if the current position is beyond the limit of the list
        if i >= len(items):
            #Wrap around and go to the position that follows the pattern
            i = i - len(items)   
        #if the current position is within the limits of the list
        else:
            #add the person at that position to the list of dead people and remove that person from the list of living people
            dead.append(items.pop(i))
            #To move to the next position, add k steps to i 
            #However since the list has one less person, we need to keep that in mind
            #and subtract 1 from k+i
            i+=k - 1  
